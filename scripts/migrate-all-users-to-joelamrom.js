/**
 * Script to migrate all data from all users to joelamrom@gmail.com
 * This will assign all families, payments, tasks, etc. from all users to joelamrom@gmail.com
 * 
 * Usage: node scripts/migrate-all-users-to-joelamrom.js
 */
...
async function migrateAllUsersToJoelamrom() {
  try {
    ...
    for (const sourceUser of usersToMigrate) {
      ...
      if (remainingFamilies > 0 || remainingTasks > 0 || remainingReports > 0) {
        ...
        console.log(`✅ ${sourceUser.email} - all data migrated`)
      }
    }
